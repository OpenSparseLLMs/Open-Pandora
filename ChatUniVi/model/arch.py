...
class ChatUniViMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images) # ([1, 256, 1024])
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if len(image_token_indices) > 1:
                ...
                image_features_to_concat = []
                for single_image_indices in temp:
                    cur_image_features = []
                    for _ in single_image_indices: 
                        cur_image_features.append(image_features[cur_image_idx])
                        cur_image_idx += 1
                    image_features_to_concat.append(torch.stack(cur_image_features, dim=0))
                for i, image_feature in enumerate(image_features_to_concat):
                    image_features = self.project(image_feature, input_type="video" if len(image_feature) > 1 else "image")
                    t, l, n = image_features.size()
                    image_features = image_features.contiguous().view(t * l, n)
                    image_features_to_concat[i] = image_features.to(device=self.device)
                    
                    
                # Get input embeddings between image token chunks
                input_ids_to_concat = [cur_input_ids[:temp[0][0]]]
                labels_to_concat = [cur_labels[:temp[0][0]]]
                for i in range(len(temp) - 1):
                    input_ids_to_concat.append(cur_input_ids[temp[i][-1] + 1:temp[i + 1][0]])
                    labels_to_concat.append(cur_labels[temp[i][-1] + 1:temp[i + 1][0]])
                input_ids_to_concat.append(cur_input_ids[temp[-1][-1] + 1:])
                input_embeds_to_concat = [self.get_model().embed_tokens(x) for x in input_ids_to_concat]
                labels_to_concat.append(cur_labels[temp[-1][-1] + 1:])
                
                # Concat input embeddings and image features alternately
                input_embeds_p, image_features_p = 0, 0
                while input_embeds_p < len(input_embeds_to_concat):
                    cur_new_input_embeds.append(input_embeds_to_concat[input_embeds_p])
                    if labels is not None:
                        cur_new_labels.append(labels_to_concat[input_embeds_p])
                    input_embeds_p += 1
                    if image_features_p < len(image_features_to_concat):
                        cur_new_input_embeds.append(image_features_to_concat[image_features_p])
                        if labels is not None:
                            cur_new_labels.append(torch.full((image_features_to_concat[image_features_p].shape[0],), IGNORE_INDEX, device=labels.device, dtype=labels.dtype))
                        image_features_p += 1
